...
Thanks for your interest in contributing to `@techsquidtv/eslint-plugin-structured-logging`! 💖
...
Please do [report an issue on the issue tracker](https://github.com/KyleTryon/eslint-plugin-structured-logging/issues/new/choose) if there's any bugfix, documentation improvement, or general enhancement you'd like to see in the repository! Please fully fill out all required fields in the most appropriate issue form.
...
With the exception of very small typos, all changes to this repository generally need to correspond to an [unassigned open issue marked as `status: accepting prs` on the issue tracker](https://github.com/KyleTryon/eslint-plugin-structured-logging/issues?q=is%3Aissue+is%3Aopen+label%3A%22status%3A+accepting+prs%22+no%3Aassignee+).
If this is your first time contributing, consider searching for [unassigned issues that also have the `good first issue` label](https://github.com/KyleTryon/eslint-plugin-structured-logging/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22+label%3A%22status%3A+accepting+prs%22+no%3Aassignee+).
...
When a PR is squash-merged, that title becomes the commit message in the repository history.
Don't worry if you get this wrong: you can always change the PR title after sending it.
Check [previously merged PRs](https://github.com/KyleTryon/eslint-plugin-structured-logging/pulls?q=is%3Apr+is%3Amerged+-label%3Adependencies+) for reference.

#### PR Titles And Releases

This repository uses [Conventional Commit](https://www.conventionalcommits.org/en/v1.0.0) titles to determine the published version of `@techsquidtv/eslint-plugin-structured-logging` and to generate release notes.
...
A scope is optional and free-form (for example, `fix(logger): ...`).

The release type is inferred from the title:
...
The same title validator runs in the local `commit-msg` hook and in the PR title workflow.
We squash merge PRs, so the PR title should be the final commit subject that appears on `main`.
